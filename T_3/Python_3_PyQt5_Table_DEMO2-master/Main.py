...
class Main(QDialog):
    ...
    def solve(self):
        ...
        if not list_information_max_num:
            ...
            self.label_max_el.setText(
                'Максимальный элемент: ' + str(list_information_max_num[0]) + ' [' +
               # Эту строку использовать
                str(list_information_max_num[1]) + ';' + str(list_information_max_num[2]) + ']')
        sum_r2 = sum_row2(self.tableWidget)
        self.label_sum.setText('Сумма: ' + str(sum_r2))
        # Эту строку использовать
        print(sum_r2)
...
def sum_row2(table_widget):
    row = 0
    col = 1
    sum_r2 = 0
    while row < table_widget.rowCount():
        sum_r2 = sum_r2 + int(table_widget.item(row, col).text())
        row += 1
    return sum_r2
